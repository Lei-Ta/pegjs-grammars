...

{
  function objType(obj){
    return Object.prototype.toString.call(obj).replace(/\[tuple (.*?)\]/g, "$1")
  }
...
message
  = n:name _? s:suffixes? _? o:(message / tuple / l:litteral)? _? arr:array? {
      result = { "_TYPE_" : "_MESSAGE_", "_NAME_" : n};
      if (s) result["_VALUE_"] = s;
      if (o) if(result["_VALUE_"]){
        result["_VALUE_"] = result["_VALUE_"].concat(o);
      } else {
        result["_VALUE_"] = o;
      }
      if (arr) result['_LOOP_'] = arr;
      return result;
    }
  / n:name 
...
array
  = _? "*" _? "(" _? es:expressions __? ")" {
    result = [];
    for (var i in es) result.push(
      { "_TYPE_" : "_ITERATION_", "_VALUE_" : es[i] }
    );
    return result; 
  }
...
suffixes
  = ( 
      _? n:( "#" { return "id";    } / ":" { return "class"; } ) s:symbol { 
        return { "_TYPE_"   : "_FIELD_", 
                 "_NAME_"   : n , 
                 "_ACTION_" : "_SET_", 
                 "_VALUE_"  : { "_TYPE_" : "_STRING_", "_CONTENT_" : s }
               }; 
      }
    )+
